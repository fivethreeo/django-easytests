...
def _get_test_labels(test_modules):
    ...
    for test_module in test_modules:
        for module in [name for _, name, _ in pkgutil.iter_modules([os.path.join(test_module, "tests")])]:
            clsmembers = pyclbr.readmodule(
                "%s.tests.%s" % (test_module, module))
            for clsname, cls in clsmembers.items():
                for method, _ in cls.methods.items():
                    if method.startswith('test_'):
                        test_labels.append(
                            '%s.%s.%s' % (test_module, clsname, method))
    return test_labels
...
'''
    ...
    def __init__(self, appname='djeasytests', test_settings={}, test_modules=None, fallback_settings=None, default_settings=global_settings, version=None):
        ...
        if fallback_settings:
            if default_settings:
                self.default_settings = GlobalSettingsWrapper(
    ...
    def run(self, thefile):
        ...
        if self.args.get('test', False):
            ...
            self.server(
                bind=self.args.get('--bind'), port=int(self.args.get('--port')))
    ...
    def isolated(self):
        ...
        test_labels = self.args.get(
            '<test-label>', '') or _get_test_labels(self.test_modules)
        if parallel:
        ...
        results = mapper(_test_in_subprocess, ((
            test_label, self.path, migrate) for test_label in test_labels))
    ...
    def test(self):
        ...
        if parallel:
            ...
            pool = multiprocessing.Pool()
            # fixme
            failures = sum(pool.map(
                _test_run_worker_settings, ([test, test_settings] for test in worker_tests)))
            return failures
        else:
    ...
    def manage(self):
        ...
        execute_from_command_line(
            [self.filename] + self.args.get('<args>', []))
